...
describe('MockGoogleMapsAPI', () => {
  let googleAPI: MockGoogleMapsAPI;
  
  beforeEach(() => {
    googleAPI = MockGoogleMapsAPI.getInstance();
  });
  ...
  describe('Route Computation', () => {
    ...
    it('should compute routes between two locations', async () => {
      ...
      expect(routes.length).toBeLessThanOrEqual(4); // Max 4 routes
      ...
      routes.forEach(route => {
        ...
        expect(route.status).toBe('CLEAR'); // Initial status
      });
    });
    ...
    it('should generate different routes for same origin-destination pair', async () => {
      ...
      if (routes.length > 1) {
        // Routes should have different names or distances
        ...
        // Should have some variation in either names or distances
    ...
    it('should handle same origin and destination', async () => {
      ...
      routes.forEach(route => {
        expect(route.distance).toBeLessThan(1000); // Less than 1km
      });
    });

    it('should calculate reasonable distances for Kent locations', async () => {
      const sevenoaks: Location = {
        latitude: 51.2719,
        longitude: 0.1904,
        address: 'Sevenoaks, Kent'
      };

      const tunbridgeWells: Location = {
        latitude: 51.1321,
        longitude: 0.2634,
      ...
      // Distance should be reasonable for this journey (approximately 20-30km)
      routes.forEach(route => {
        expect(route.distance).toBeGreaterThan(15000); // At least 15km
        expect(route.distance).toBeLessThan(50000); // Less than 50km
        expect(route.staticDuration).toBeGreaterThan(900); // At least 15 minutes
        expect(route.staticDuration).toBeLessThan(3600); // Less than 1 hour
      });
    });
    ...
    it('should provide consistent results for same input', async () => {
      ...
      // Route names and static durations should be similar (allowing for some variation)
      routes1.forEach((route1, index) => {
        const route2 = routes2[index];
        expect(route1.name).toBe(route2.name);
        // Static duration should be within 10% variation
  ...
  describe('Singleton Pattern', () => {
    it('should return the same instance', () => {
      const instance1 = MockGoogleMapsAPI.getInstance();
      const instance2 = MockGoogleMapsAPI.getInstance();
...
describe('MockTomTomTrafficAPI', () => {
  let tomtomAPI: MockTomTomTrafficAPI;
  
  beforeEach(() => {
    tomtomAPI = MockTomTomTrafficAPI.getInstance();
  });
  ...
  describe('Singleton Pattern', () => {
    it('should return the same instance', () => {
      const instance1 = MockTomTomTrafficAPI.getInstance();
      const instance2 = MockTomTomTrafficAPI.getInstance();
  ...
  describe('Dynamic Traffic Updates', () => {
    it('should have dynamic traffic conditions that change over time', (done) => {
      ...
      // Wait for potential updates (traffic conditions update every 30 seconds)
      // We'll use a shorter timeout for testing
      setTimeout(() => {
        const updatedConditions = tomtomAPI.getCurrentConditions();
        
        // Conditions might change (but this is probabilistic, so we just check structure)
